...
class ExtractedAutoEncoder(nn.Module):
    def __init__(self, original_model, num_layers):
        super().__init__()
        
        # 提取前 num_layers - 1 次的编码器和解码器
        self.encoders = nn.ModuleList(original_model.encoders[:num_layers - 1])
        self.decoders = nn.ModuleList(original_model.decoders[:num_layers - 1])
        
        # 提取最后一次的编码器
        self.last_encoder = original_model.encoders[num_layers - 1]

    def forward(self, encoder_input):
        encoder_output = None
        decoder_output = None
        
        # 逐层通过前 num_layers - 1 次的编码器和解码器
        for encoder, decoder in zip(self.encoders, self.decoders):
        ...
        # 通过最后一次编码器
        last_encoder_output = self.last_encoder(decoder_output)

        return last_encoder_output  # 返回最后一次编码器的输出
